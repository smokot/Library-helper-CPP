...
public:
	...
	{
	    int blockWidth = search[0].size(); // ширина блока (кол-во столбцов)
	    int blockHeight = search.size(); // высота блока (кол-во строк)

	    for (int i = 0; i < source.size(); i++)
	    {
		if (source[i] == search[0][0]) // если нашли первый символ блока
		{
		    // проверяем, есть ли остальные символы блока в строке
		    bool found = true;
		    ...
		    if (found) // если нашли первую строку блока
		    {
			// проверяем, есть ли остальные строки блока в следующих строках
			bool blockFound = true;
			for (int k = 1; k < blockHeight; k++)
			{
			    int nextLineStart = i + (k * lineLength); // начало следующей строки
	...
	float Monochrome(COLORREF cr)
	{
		...
		// êâàäðàò ðàññòîÿíèÿ äî ïåðâîé îïîðíîé òî÷êè
		...
		// êâàäðàò ðàññòîÿíèÿ äî âòîðîé îïîðíîé òî÷êè
		...
		// êâàäðàò ðàññòîÿíèÿ äî òðåòüåé îïîðíîé òî÷êè
		...
		// ïðîâåðÿåì êàêîå ðàññòîÿíèå ìåíüøå
	...
	//-------------------------------------------------------------------------------------------------
	// ïðåîáðàçîâàíèè ãîðèçîíòàëüíîé ïèêñåëüíîé ëèíèè â òåêñòîâóþ ñòðîêó
	//-------------------------------------------------------------------------------------------------
	void TranslateLine(COLORREF* line, int cx, std::string& str)
	{
		...
		// ïðîáåãàåìñÿ ïî âñåì ïèêñåëÿõ îäíîé ãîðèçîíòàëüíîé ëèíèè êàðòèíêè
		for (int i = 0; i < cx; i++)
		{
			// äîáàâëÿåì â ñòðîêó '0' èëè '1' â çàâèñèìîñòè îò öâåòà ïèêñåëÿ (çàìåíèòü ' ' íà '0')
	...
	//-------------------------------------------------------------------------------------------------
	// ïðåîáðàçîâàíèå êàðòèíêè â òåêñò
	//-------------------------------------------------------------------------------------------------
	void Bitmap_To_Txt(HBITMAP hbmp, std::string& text)
	{
		...
		// áóôåð ïîä ãîðèçîíòàëüíóþ ëèíèþ êàðòèíêè
		...
		// ïðîáåãàåìñÿ ïî âñåì ãîðèçîíòàëüíûì ëèíèÿì êàðòèíêè
		for (int i = 0; i < bm.bmHeight; i++)
		{
			// ÷òåíèå îäíîé ãîðèçîíòàëüíîé ëèíèè (bm.bmHeight-i-1 - âåðòèêàëüíîå îòðàæåíèå)
			GetDIBits(hDC, hbmp, bm.bmHeight - i - 1, 1, line.data(), &bmi, DIB_RGB_COLORS);

			// ôîðìèðîâàíèå èç ëèíèè òåêñòîâîé ñòðîêè
			std::string str;
			TranslateLine((COLORREF*)line.data(), bm.bmWidth, str);

			// ôîðìèðîâàíèå âñåãî òåêñòà
			text += str + "\n";
		}
...
float main_pressed = clock(); // Âðåìÿ íàæàòèÿ
float time_pressed = clock(); // Âðåìÿ íàæàòèÿ
float save_time = 0;
int counter_start = 0;
bool hidden_press = false;
vector<int>keys; // Êëàâèøè
vector<int>time_to_press; // Âðåìÿ íàæàòèé êëàâèø
bool flag_active = true; // Êîíòðîëü çàïèñè íàæàòèé
PKBDLLHOOKSTRUCT pHook; // ÊÍÎÏÊÀ 
